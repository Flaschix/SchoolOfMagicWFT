...
export class GameScene3 extends Phaser.Scene {
    ...
    createCollision() {
        ...
        const bodyRightMiddleTable = this.matter.add.fromVertices(1286 + 90, 1189 + 90, '325.5 85.5 132 267 1.5 160 24.5 142 35 146 190.5 1 298 78 309.5 70', { label: '1', isStatic: true });
        const bodyRightMiddleShell = this.matter.add.fromVertices(1760 + 120, 990 + 210, '119.087 428.01 141.796 450 154.916 428.01 154.916 395.281 164 380.962 164 53.6731 77.7059 1 77.7059 71.0604 1 129.359 11 330 119.087 406.532', { label: '1', isStatic: true });
        ...
        const bodyLeftMiddleTable = this.matter.add.fromVertices(540 + 90, 1050 + 90, '66 234 11.5 213.5 6 213.5 6 205.5 6 189.5 6 154.5 1 134 82 46 101 30.5 134 20 145 8.5 167 1 190.5 11.5 199 11.5 213 30.5 208.5 46 216 70 226.5 107.5 146.5 180 113.5 213.5', { label: '1', isStatic: true });
        ...
        const bodyLeftMiddleShell = this.matter.add.fromVertices(125 + 90, 1020 + 183, '40 393.5 16.5 411 13.5 409.5 4.5 393.5 4.5 381 4.5 353.5 1 345.5 1 214.5 1 170.5 1 106.5 34 76 38 46.5 95 1 150.5 38 138 76 155.5 92 160.5 92 168 103 155.5 123 150.5 315.5 144.5 322 138 315.5 34 381', { label: '1', isStatic: true });
        const bodyLeftBottomTable = this.matter.add.fromVertices(450 + 80, 1622 + 52, '164.5 63.5 144 128.5 107.5 151 62 153.5 25.5 144 0.5 99 0.5 63.5 31.5 22.5 93.5 1 140 22.5', { label: '1', isStatic: true });
